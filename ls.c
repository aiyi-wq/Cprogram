/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: Mon 27 Jul 2020 10:25:24 PM CST
 ************************************************************************/
//-l -a
...
void show_files(char names[][MAXNAME], int cnt, int row, int col) {
    ...
    struct stat tmp_st;
    for (int i = 0; i < col; i++) {
        for (int j = (i * row); j < (i + 1) * row && i < cnt; j++) {
            if (wide[i] < strlen(names[j])) wide[i] = strlen(names[j]);
        }
    }
    for (int i = 0; i < row; i++) {
        for (int j = i; j < i + (row * cnt) && j < cnt; j += row) {
            lstat(names[j], &tmp_st);
            update_color(tmp_st.st_mode);
            int wide_tmp = wide[j / row];
            printf("\033[%d;%dm%-*s\033[0m", bg_c, fg_c,  wide_tmp + 2, names[j]);//- : 左对齐 *: 宽度 
        }
        printf("\n");
    }
}
...
void do_ls(char *dir) {
    ...
    if ((dirp = opendir(dir)) == NULL) {
        ...
        if (l_flag == 1) {
            ...
            int row, col;
            //&row &col是传出参数（&是取地址符，在size_window函数里row和col的值会存在地址里），names是数组，当作为传入参数时会退化成指针，因此需要用cnt指示它的大小
...
void show_info(char *filename, struct stat *st) {
    ...
    printf("%.16s ", 4 + ctime(&st->st_mtime));
